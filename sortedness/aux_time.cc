...
#ifdef __USE_GETTIMEOFDAY
  std::cout << "CLK: " << clk.tv_sec << " s + " << clk.tv_usec
            << " ns = " << ((double)clk.tv_sec + (double)clk.tv_usec / 1000000)
            << " sec" << std::endl;
#else
  std::cout << "CLK: " << clk.tv_sec << " s + " << clk.tv_nsec << " ns = "
            << ((double)clk.tv_sec + (double)clk.tv_nsec / 1000000000) << " sec"
            << std::endl;
#endif
}
...
#else
  return 1000000 * (clk2.tv_sec - clk1.tv_sec) +
         (clk2.tv_nsec - clk1.tv_nsec) / 1000;
#endif
}

long getclock_diff_ns(my_clock clk1, my_clock clk2) {
#ifdef __USE_GETTIMEOFDAY
  return 1000000000 * (clk2.tv_sec - clk1.tv_sec) +
         (clk2.tv_usec - clk1.tv_usec);
#else
  return 1000000000 * (clk2.tv_sec - clk1.tv_sec) +
         (clk2.tv_nsec - clk1.tv_nsec);
#endif
}

double getclock_diff_s(my_clock clk1, my_clock clk2) {
#ifdef __USE_GETTIMEOFDAY
  return (double)(clk2.tv_sec - clk1.tv_sec) +
         (double)(clk2.tv_usec - clk1.tv_usec) / 1000000;
#else
  return (double)(clk2.tv_sec - clk1.tv_sec) +
         (double)(clk2.tv_nsec - clk1.tv_nsec) / 1000000000;
#endif
}
