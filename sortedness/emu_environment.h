...
public:
  ...
  uint16_t
      compaction_pri;  // C | 1:kMinOverlappingRatio, 2:kByCompensatedSize,
  ...
  uint16_t memtable_factory;   // 1: skiplist, 2: vector, 3: hash skiplist, 4:
                               // hash linklist

  // Compaction options
  long target_file_size_base; // file size in level base (usually level 1)
  bool level_compaction_dynamic_level_bytes; // related to BG-merging -- ensures
                                             // last level is T times larger
                                             // than the previous level
  uint16_t compaction_style;                 // 1: kCompactionStyleLevel, 2:
                                             // kCompactionStyleUniversal, 3:
  // kCompactionStyleFIFO, 4: kCompactionStyleNone
  bool disable_auto_compactions; // manual comapction (when compactions_style =
                                 // 4, i.e., kCompactionStyleNone)
  uint16_t compaction_filter;    // 0: nullptr, 1: invoking custom compaction
                                 // filter, if any
  uint16_t compaction_filter_factory;  // 0: nullptr, 1: invoking custom
  ...
  int level0_file_num_compaction_trigger; // [RDB_default: 4] number of files
                                          // that triggers compaction in level0,
                                          // must <=
                                          // level0_slowdown_writes_trigger <=
                                          // level0_stop_writes_trigger
  int level0_slowdown_writes_trigger;     //[RDB_default: 20] must
                                      //>=level0_file_num_compaction_trigger &&
                                      //<= level0_stop_writes_trigger
  int level0_stop_writes_trigger;  // [RDB_default: 36] must >=
                                   // level0_slowdown_writes_trigger >=
                                   // level0_file_num_compaction_trigger
  ...
  int max_compaction_bytes; // target_file_size_base * 25, limit number of bytes
                            // in one compaction to be lower than this
                            // threshold. But it’s not guaranteed.
  long max_bytes_for_level_base; // capacitiy for level base(usually level 1)
  int merge_operator;            // TBC
  uint64_t
      soft_pending_compaction_bytes_limit; // All writes will be slowed down to
                                           // at least delayed_write_rate if
                                           // estimated [RDB_default: 64 *
                                           // 1073741824ull] bytes needed to be
                                           // compaction exceed this threshold.
  uint64_t hard_pending_compaction_bytes_limit; // All writes are stopped if
                                                // estimated bytes needed to be
                                                // compaction exceed this
                                                // threshold.[RDB_default: 256 *
                                                // 1073741824ull]
  int periodic_compaction_seconds;
  bool use_direct_io_for_flush_and_compaction;
  int num_levels; // Maximum number of levels that a tree may have [RDB_default:
                  // 7]
  ...
  uint16_t data_block_index_type; // 1: kDataBlockBinarySearch, 2:
                                  // kDataBlockBinaryAndHash
  uint16_t
      index_type; // 1: kBinarySearch, 2: kHashSearch, 3: kTwoLevelIndexSearch
  bool partition_filters;
  int metadata_block_size;             // TBC| Currently deprecated by
  ...
  // Compression options
  uint16_t
      compression; // 1: kNoCompression, 2: kSnappyCompression, 3:
                   // kZlibCompression, 4: kBZip2Compression, 5:
                   // kLZ4Compression, 6: kLZ4HCCompression, 7:
                   // kXpressCompression, 8: kZSTD, 9: kZSTDNotFinalCompression,
                   // 10: kDisableCompressionOption

  // ReadOptions
  bool verify_checksums; // TBC
  bool fill_cache; // data block/index block read: this iteration will not be
                   // cached
  int iter_start_seqnum;       // TBC
  bool ignore_range_deletions; // TBC
  uint16_t read_tier; // 1: kReadAllTier, 2: kBlockCacheTier, 3: kPersistedTier,
                      // 4: kMemtableTier

  // WriteOptions
  bool low_pri; // every insert is less important than compaction
  bool sync; // make every write wait: sync with log (so we see real perf impact
             // of insert)
  bool disableWAL;
